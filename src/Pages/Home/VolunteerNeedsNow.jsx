...
const VolunteerNeedsNow = ({ posts = [] }) => {
      ...
      >
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300 mb-1">
            <strong className="dark:text-gray-200">Category:</strong> {post.category}
          </p>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
            <strong className="dark:text-gray-200">Deadline:</strong>{" "}
            {new Date(post.deadline).toLocaleDateString()}
          </p>
          <Link to={`/volunteer/${post._id}`} className="mt-auto">
            <button className="w-full bg-[#6594aa] text-white px-4 py-2 rounded hover:bg-[#73c7ee] transition duration-300">
              View Details
            </button>
          </Link>
        </div>
  ...
  <div className="text-center mt-12">
    <Link to="/volunteer">
      <button className="bg-[#122e2c] text-white px-6 py-2 rounded hover:bg-[#1e7570] transition duration-300">
        See All
      </button>
    </Link>
  </div>
